...
async def bootstrap_session(session, sessionId):
    ...
    async with session.post(f'https://visualizedata.ucop.edu/vizql/t/Public/w/TransferbyCCM/v/ByMajorName/bootstrapSession/sessions/{sessionId}', data=form_data) as response:
        if response.status != 200:
            raise RuntimeError
        ...
        optionsStr = first_data['worldUpdate']['applicationPresModel']['workbookPresModel']['dashboardPresModel']['zones']['20']['presModelHolder']['visual']['filtersJson']
...
async def generate_tempfile(session, sessionId, sheetdocId):
    with aiohttp.MultipartWriter('form-data') as mp:
        ...
        async with session.post(f'https://visualizedata.ucop.edu/vizql/t/Public/w/TransferbyCCM/v/ByMajorName/sessions/{sessionId}/commands/tabsrv/export-crosstab-to-csvserver', data=mp) as response:
            if response.status != 200:
                print(response)
            ...
            tempfileKey = data['vqlCmdResponse']['layoutStatus']['applicationPresModel']['presentationLayerNotification'][0]['presModelHolder']['genFileDownloadPresModel']['tempfileKey']
...
async def switch_campus(session, sessionId, idx):
    with aiohttp.MultipartWriter('form-data') as mp:
        ...
        for key, value in data.items():
            ...
            async with session.post(f'https://visualizedata.ucop.edu/vizql/t/Public/w/TransferbyCCM/v/ByMajorName/sessions/{sessionId}/commands/tabdoc/categorical-filter-by-index', data=mp) as response:
                if response.status != 200:
                    raise RuntimeError
...
async def do_scrape():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i, cmp_data in enumerate(options):
            ...
            tempfileKey = await generate_tempfile(session, sessionId, '{2193A602-EAF6-4CF7-B7CF-B9BB4F163F60}')

            print('generated temporary download link')

            async with session.get(f'https://visualizedata.ucop.edu/vizql/t/Public/w/TransferbyCCM/v/ByMajorName/tempfile/sessions/{sessionId}/?key={tempfileKey}&keepfile=yes&attachment=yes') as response:
                if response.status != 200:
                    raise RuntimeError
